...
var _ = Describe("Projection Controller (integration)", func() {
	...
	Context("Create path", func() {
		...
		BeforeEach(func() {
			// Namespaced Projection writes into its own namespace, so source
			// and destination share `ns` here. Cross-namespace mirroring is
			// the cluster-scoped sibling's job.
			ns = uniqueNS("mirror-create")
			...
			proj := &projectionv1.Projection{
				...
				Spec: projectionv1.ProjectionSpec{
					...
					Destination: projectionv1.ProjectionDestination{Name: destCMName},
		...
		It("projects the source to the destination with overlay applied", func() {
			...
			// UID label is the watch hint — assert it's stamped with the
			// owning Projection's UID.
			proj := &projectionv1.Projection{}
			Expect(k8sClient.Get(ctx, projKey, proj)).To(Succeed())
			Expect(dst.Labels).To(HaveKeyWithValue(ownedByUIDLabel, string(proj.UID)))
			...
			// Status carries the resolved destination name (printcolumn).
			Expect(proj.Status.DestinationName).To(Equal(destCMName))

			// Regression: pinned source.version forms must keep an empty
			// SourceResolved.message — only unpinned (version omitted)
			// populates it.
		...
		It("updates the destination when the source changes", func() {
			...
			src := &corev1.ConfigMap{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: sourceCMName, Namespace: ns}, src)).To(Succeed())
		...
		It("populates SourceResolved.message with the resolved version when source.version is unpinned", func() {
			depNS := uniqueNS("unpinned")
			ensureNamespace(depNS)
			...
			// Projection lives in the same namespace as the source so the
			// destination (which is implicitly proj.Namespace) collides with
			// the source's name unless we rename — pick a destination name
			// distinct from "src-dep".
			unpinnedKey := types.NamespacedName{Name: "unpinned-proj", Namespace: depNS}
			proj := &projectionv1.Projection{
				ObjectMeta: metav1.ObjectMeta{Name: unpinnedKey.Name, Namespace: unpinnedKey.Namespace},
				Spec: projectionv1.ProjectionSpec{
					Source: projectionv1.SourceRef{
						Group: "apps", Kind: "Deployment",
						Name: "src-dep", Namespace: depNS,
					},
					Destination: projectionv1.ProjectionDestination{Name: "dst-dep"},
				},
			}
			Expect(k8sClient.Create(ctx, proj)).To(Succeed())
	...
	Context("Deletion path", func() {
		...
		It("removes the owned destination and finalizer when the Projection is deleted", func() {
			...
			// Destination was created and owned (annotation + UID label).
			dst := &corev1.ConfigMap{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: destCMName, Namespace: ns}, dst)).To(Succeed())
			Expect(dst.Annotations).To(HaveKeyWithValue(ownedByAnnotation, ns+"/"+projName))
			Expect(dst.Labels).To(HaveKeyWithValue(ownedByUIDLabel, string(proj.UID)))
		...
		It("refuses to project a source missing the projectable annotation in allowlist mode", func() {
			...
			proj := &projectionv1.Projection{
				...
				Spec: projectionv1.ProjectionSpec{
					...
					Destination: projectionv1.ProjectionDestination{Name: "dst-never-written"},
				},
			}
			Expect(k8sClient.Create(ctx, proj)).To(Succeed())
		...
		It("honors opt-out (false) even in permissive mode, garbage-collecting an existing destination", func() {
			...
			proj := &projectionv1.Projection{
				...
				Spec: projectionv1.ProjectionSpec{
					...
					Destination: projectionv1.ProjectionDestination{Name: "mirrored-cm"},
				},
			}
			Expect(k8sClient.Create(ctx, proj)).To(Succeed())
			DeferCleanup(func() { deleteProjection(projKey) })
	...
	Context("Source deletion", func() {
		It("cleans up the destination when the source is deleted", func() {
			id := nextID()
			// Source and Projection share a namespace (destination is the
			// Projection's own namespace). The Projection renames the dest
			// so it doesn't collide with the source name.
			ns := uniqueNS("mirror-srcdel")
			projKey := types.NamespacedName{Name: "p-srcdel-" + id, Namespace: ns}
			srcName := "src-cm-" + id
			dstName := "dst-cm-" + id
			...
			Expect(k8sClient.Get(ctx,
				types.NamespacedName{Name: dstName, Namespace: ns}, &corev1.ConfigMap{})).To(Succeed())
			...
			Eventually(func() error {
				return k8sClient.Get(ctx,
					types.NamespacedName{Name: dstName, Namespace: ns}, &corev1.ConfigMap{})
...
var _ = Describe("Shared source watch (integration with manager)", Ordered, func() {
	...
	It("two Projections sharing a source GVK register a single watch and both reconcile", func() {
		id := nextID()
		// Two Projections live in the same namespace as the source (each
		// only ever writes into its own namespace) and rename to different
		// destination names so they don't fight over the same target.
		ns := uniqueNS("sharedwatch")
		srcName := "src-cm-" + id
		dstName1 := "dst-cm-1-" + id
		dstName2 := "dst-cm-2-" + id
		proj1Key := types.NamespacedName{Name: "p-sw1-" + id, Namespace: ns}
		proj2Key := types.NamespacedName{Name: "p-sw2-" + id, Namespace: ns}
		...
		for _, spec := range []struct {
			...
			proj := &projectionv1.Projection{
				...
				Spec: projectionv1.ProjectionSpec{
					...
					Destination: projectionv1.ProjectionDestination{Name: spec.dstName},
				},
			}
			Expect(k8sClient.Create(ctx, proj)).To(Succeed())
		}
		...
		for _, dstName := range []string{dstName1, dstName2} {
			nameCopy := dstName
			Eventually(func() string {
				dst := &corev1.ConfigMap{}
				_ = k8sClient.Get(ctx,
					types.NamespacedName{Name: nameCopy, Namespace: ns}, dst)
				return dst.Data["version"]
			}, 10*time.Second, 200*time.Millisecond).Should(Equal("1"),
				"destination missing or wrong version: %s", nameCopy)
		}
		...
		Expect(k8sClient.Get(ctx,
			types.NamespacedName{Name: srcName, Namespace: ns}, updated)).To(Succeed())
	...
	It("a pinned and an unpinned Projection on the same source share one watch entry", func() {
		id := nextID()
		// Both Projections live in the source namespace and rename their
		// destinations so two single-target writes in the same namespace
		// don't collide.
		ns := uniqueNS("mixedwatch")
		ensureNamespace(ns)

		srcName := "shared-dep-" + id
		dstNamePinned := "dst-pinned-" + id
		dstNameUnpinned := "dst-unpinned-" + id
		pinnedKey := types.NamespacedName{Name: "pinned-proj-" + id, Namespace: ns}
		unpinnedKey := types.NamespacedName{Name: "unpinned-proj-" + id, Namespace: ns}
		...
		for _, spec := range []struct {
			key     types.NamespacedName
			version string // "" means unpinned (RESTMapper picks preferred)
			dstName string
		}{
			{pinnedKey, "v1", dstNamePinned},
			{unpinnedKey, "", dstNameUnpinned},
		} {
			proj := &projectionv1.Projection{
				ObjectMeta: metav1.ObjectMeta{Name: spec.key.Name, Namespace: spec.key.Namespace},
				Spec: projectionv1.ProjectionSpec{
					Source: projectionv1.SourceRef{
						Group: "apps", Version: spec.version, Kind: "Deployment",
						Name: srcName, Namespace: ns,
					},
					Destination: projectionv1.ProjectionDestination{Name: spec.dstName},
				},
		...
		for _, dstName := range []string{dstNamePinned, dstNameUnpinned} {
			nameCopy := dstName
			Eventually(func() error {
				d := &appsv1.Deployment{}
				return k8sClient.Get(ctx,
					client.ObjectKey{Namespace: ns, Name: nameCopy}, d)
			}, 10*time.Second, 200*time.Millisecond).Should(Succeed(),
				"destination Deployment missing: %s", nameCopy)
		}
		...
		Expect(k8sClient.Get(ctx,
			client.ObjectKey{Namespace: ns, Name: srcName}, updated)).To(Succeed())
		...
		for _, dstName := range []string{dstNamePinned, dstNameUnpinned} {
			...
			Eventually(func() string {
				...
				if err := k8sClient.Get(ctx,
					client.ObjectKey{Namespace: ns, Name: nameCopy}, d); err != nil {
					return ""
				}
	...
	It("accepts and reconciles a source whose name contains a dot (Finding C regression)", func() {
		// Single namespace (source + projection + destination all live here).
		// The Projection renames the dotted-name source to a different
		// dotted-name destination so the two don't collide.
		ns := uniqueNS("fc")
		ensureNamespace(ns)

		const dottedSrcName = "app.config-v2"
		const dottedDstName = "app.config-v2-mirror"
		projKey := types.NamespacedName{Name: "finding-c-" + nextID(), Namespace: ns}
		DeferCleanup(deleteProjection, projKey)
		...
		// subdomain-format name on both source and destination.
		proj := &projectionv1.Projection{
			ObjectMeta: metav1.ObjectMeta{
				Name:      projKey.Name,
			...
			Spec: projectionv1.ProjectionSpec{
				...
				Destination: projectionv1.ProjectionDestination{Name: dottedDstName},
			},
		}
		Expect(k8sClient.Create(ctx, proj)).To(Succeed())

		// The reconciler should pick it up and write a destination with the
		// renamed dotted name in the same namespace.
		Eventually(func() error {
			var got corev1.ConfigMap
			return k8sClient.Get(ctx, client.ObjectKey{Namespace: ns, Name: dottedDstName}, &got)
		}, 10*time.Second, 200*time.Millisecond).Should(Succeed())
		...
		var got corev1.ConfigMap
		Expect(k8sClient.Get(ctx, client.ObjectKey{Namespace: ns, Name: dottedDstName}, &got)).To(Succeed())
