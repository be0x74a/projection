...
.PHONY: update-crd-golden  # Regenerate the CRD schema golden file. Use after intentional api/v1/*.go changes.
...
sync-chart-crds: manifests ## Copy generated CRDs into the Helm chart's crds/ directory.
	...
	cp config/crd/bases/projection.sh_clusterprojections.yaml charts/projection/crds/clusterprojections.projection.sh.yaml
...
helm-test: ## Run helm-unittest on the chart. Installs the plugin on first run.
	@if ! helm plugin list | grep -q unittest; then \
	    ...
	    helm plugin install --verify=false --version v1.0.3 https://github.com/helm-unittest/helm-unittest; \
	fi
	helm unittest charts/projection

.PHONY: docs-ref
docs-ref: crd-ref-docs ## Regenerate docs/api-reference.md from api/v1 types + kubebuilder markers.
	$(CRD_REF_DOCS) \
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name projection-builder
	$(CONTAINER_TOOL) buildx use projection-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm projection-builder
	rm Dockerfile.cross
